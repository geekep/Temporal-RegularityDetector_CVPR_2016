...
class TemporalRegularityDetector(object):
    ...
    def _build(self):
        ...
        self._regularity = \
            1. - (self._pixel_error - pixel_error_min) / pixel_error_max
    ...
    def _deconv2d(self, input_, output_shape, output_dim,
        ...
        with tf.variable_scope(name):
            ...
            deconv = tf.nn.conv2d_transpose(input_, k, output_shape, [1, s_h, s_w, 1], "VALID")
...
if __name__ == '__main__':
    with tf.Session() as sess:
        input_shape = [10, 227, 227, 10]
        trd = TemporalRegularityDetector(sess, input_shape)
        input = tf.random_normal(shape=input_shape)
        trd.fit(input)
